...
- **plover chick data** in which each row is a single chick. 
...
Define the variables of the `plover_ind` correctly.  `easting`, `northing`, and `prop_male` need to recognized as numeric; `chickID`, `nest`, `nestID`, `species`, `sex`, and `year` need to recognized as factors; and `hatch_date` needs to be recognized as a date.

```{r}
plover_chick[,c("easting", "northing")] <- 
  lapply(plover_chick[,c("easting", "northing")], as.numeric)

plover_chick[,c("chickID", "nestID", "species", "sex", "year")] <- 
  lapply(plover_chick[,c("chickID", "nestID", "species", "sex", "year")], as.factor)
```
